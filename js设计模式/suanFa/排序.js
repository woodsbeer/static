...
{
    //重写插入排序，他是对一个已经排序好的部分，后面紧跟的那个和最后一个比大小，
    //如果比较的过程中已经比排序好的数大了，可以终止当前这个循环，因为这表示已经找到应该插入的点了
    function insertSort(arr) {
        for (let i = 1; i < arr.length; i++) {
            //j定义在外面，因为循环内修改后还要再外面根据j插入被比较的未排序数
            let j = i-1;
            //内部循环的移动过程中，arr【i】对应的值可能会被改变，所以提前存起来
            let val = arr[i];
            //arr[i]就是被比较的那个，我们这个关于j的内部循环就是为了移动排序好的数组
            for (; j >=0; j--) {
                //把排序好的数组后移，同时下标j继续前移,继续比较
                if (arr[j] > val) {
                    //每次都是当前j指向的值和那个未排序的被比较值比较
                    arr[j + 1] = arr[j];
                }
                else
                    //如果前面有序数组的最后面后比被比较的数小，就不用继续左移比较了
                    break
            }
            //移动完以后要做的是插入
            arr[j+1] =val;
    ...
    console.log(insertSort([1, 5, 3, 76, 2]),'insertSort');
}
{
    //冒泡排序
